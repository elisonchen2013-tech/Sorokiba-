...
function technologyCustomizationFields(v={}){
 const x=v||{}; const util=x.utility||'Celular'; const effect=Number(x.effect)||0;
 return '<div class="technology-customizer"><strong>💻 Personalizar tecnologia</strong><div class="technology-preview"><div class="tech-device" id="techPreview">📱</div><div><b>O dono define a utilidade</b><small>Escolha o que a tecnologia faz e o bônus que ela dá.</small></div></div><div class="form-row-2"><label>Tipo<select name="techCategory"><option '+(util==='Celular'?'selected':'')+'>Celular</option><option '+(util==='Computador'?'selected':'')+'>Computador</option><option '+(util==='Tablet'?'selected':'')+'>Tablet</option><option '+(util==='Relógio inteligente'?'selected':'')+'>Relógio inteligente</option><option '+(util==='Console'?'selected':'')+'>Console</option><option '+(util==='Outro'?'selected':'')+'>Outro</option></select></label><label>Marca<input name="techBrand" maxlength="50" value="'+esc(x.brand||'')+'"></label><label>Versão<input name="techVersion" maxlength="30" value="'+esc(x.version||'1.0')+'"></label><label>Cor<input name="techColor" type="color" value="'+(x.color||'#5b6cff')+'"></label></div><label>Utilidade da tecnologia<input name="techUtility" maxlength="100" value="'+esc(x.utilityDescription||'Ex.: aumenta energia')+'"></label><label>Bônus escolhido pelo dono (%)<input name="techEffect" type="number" min="0" max="20" step="1" value="'+effect+'"></label><label>Especificações<textarea name="techSpecs" maxlength="180">'+esc(x.specs||'')+'</textarea></label></div>';
}
function readTechnologyCustomization(f){return{category:f.techCategory?.value,utility:f.techCategory?.value,brand:f.techBrand?.value,version:f.techVersion?.value,specs:f.techSpecs?.value,color:f.techColor?.value,utilityDescription:f.techUtility?.value,effect:Math.max(0,Math.min(20,Number(f.techEffect?.value)||0))}}
function clothingCustomizationFields(v={}){
 const x=v||{};
 return '<div class="clothing-customizer"><strong>👕 Personalizar roupa</strong><div class="clothing-preview"><div class="shirt-preview" style="--shirt:'+(x.primaryColor||'#222222')+';--shirt2:'+(x.secondaryColor||'#eeeeee')+'"><span>'+(esc(x.logo||'')||'SOROKIBA')+'</span></div><div><b>Monte a roupa antes de vender</b><small>Escolha modelo, cores, estampa e tamanho.</small></div></div><div class="form-row-2"><label>Categoria<select name="clothCategory"><option '+(x.category==='Camiseta'?'selected':'')+'>Camiseta</option><option '+(x.category==='Calça'?'selected':'')+'>Calça</option><option '+(x.category==='Jaqueta'?'selected':'')+'>Jaqueta</option><option '+(x.category==='Boné'?'selected':'')+'>Boné</option><option '+(x.category==='Tênis'?'selected':'')+'>Tênis</option><option '+(x.category==='Acessório'?'selected':'')+'>Acessório</option></select></label><label>Estilo<input name="clothStyle" maxlength="40" value="'+esc(x.style||'Casual')+'"></label><label>Tamanho<select name="clothSize"><option '+(x.size==='P'?'selected':'')+'>P</option><option '+(!x.size||x.size==='M'?'selected':'')+'>M</option><option '+(x.size==='G'?'selected':'')+'>G</option><option '+(x.size==='GG'?'selected':'')+'>GG</option></select></label><label>Estampa<input name="clothPattern" maxlength="40" value="'+esc(x.pattern||'Lisa')+'"></label><label>Cor principal<input name="clothPrimaryColor" type="color" value="'+(x.primaryColor||'#222222')+'"></label><label>Cor secundária<input name="clothSecondaryColor" type="color" value="'+(x.secondaryColor||'#eeeeee')+'"></label></div><label>Logo/marca<input name="clothLogo" maxlength="50" value="'+esc(x.logo||'')+'"></label></div>';
}
function readClothingCustomization(f){return{category:f.clothCategory?.value,style:f.clothStyle?.value,size:f.clothSize?.value,pattern:f.clothPattern?.value,primaryColor:f.clothPrimaryColor?.value,secondaryColor:f.clothSecondaryColor?.value,logo:f.clothLogo?.value}}
function productCustomizationForForm(f){const box=f.querySelector('#productCustomizationBox');if(!box)return;const type=f.type?.value;if(type==='tecnologia')box.innerHTML=technologyCustomizationFields();else if(type==='roupa')box.innerHTML=clothingCustomizationFields();else if(type==='veiculo'){vehicleFieldsForForm(f);return}else box.innerHTML='';}

function companyProductFields(){return '<label>Nome do produto<input name="productName" maxlength="80" required></label><label>Descrição<textarea name="productDescription" maxlength="300"></textarea></label><div class="form-row-2"><label>Preço (R$)<input name="price" type="number" min="0.01" max="1000000" step="0.01" required></label><label>Tipo<select name="type" onchange="toggleCompanyEffects(this.value);vehicleFieldsForForm(this.form);productCustomizationForForm(this.form)"><option value="consumivel">Consumível</option><option value="equipamento">Equipamento</option><option value="tecnologia">Tecnologia</option><option value="veiculo">Veículo</option><option value="roupa">Roupa</option><option value="decoracao">Decoração</option></select></label></div><div id="companyEffectsBox" class="company-effects-form"><strong>Efeitos ao usar</strong><div class="form-row-2"><label>🍽️ Fome<input name="hunger" type="number" min="-100" max="100" value="0"></label><label>💧 Hidratação<input name="hydration" type="number" min="-100" max="100" value="0"></label><label>⚡ Energia<input name="energy" type="number" min="-100" max="100" value="0"></label><label>❤️ Vida<input name="life" type="number" min="-100" max="100" value="0"></label></div></div><div id="productCustomizationBox"></div><label>Emoji do produto <input name="emoji" maxlength="8" placeholder="📦"></label><label>Foto do produto <span style="opacity:.65">(opcional)</span><input name="image" type="file" accept="image/png,image/jpeg,image/webp,image/gif"></label>'}
...
function characterSvg(c){
 ...
 const hairMap={
 curto:'<path d="M35 48C34 25 46 10 60 10C76 10 88 25 86 49C80 40 71 34 60 34C49 34 41 40 35 48Z" fill="url(#hair)"/><path d="M40 29Q50 15 61 15Q72 15 81 29" fill="none" stroke="#fff" stroke-opacity=".2" stroke-width="3" stroke-linecap="round"/><path d="M44 22Q59 12 75 22" fill="none" stroke="#fff" stroke-opacity=".1" stroke-width="2" stroke-linecap="round"/>',
 medio:'<path d="M33 53C31 25 44 9 60 9C78 9 90 26 87 55L82 82L72 52C69 40 65 34 60 34C55 34 50 40 47 52L38 82Z" fill="url(#hair)"/><path d="M39 55Q42 70 39 78M81 55Q78 70 81 78" fill="none" stroke="#fff" stroke-opacity=".15" stroke-width="3" stroke-linecap="round"/>',
 longo:'<path d="M32 55C30 25 44 8 60 8C78 8 91 26 88 57L85 108L72 101L69 51C66 39 64 34 60 34C55 34 51 40 48 52L45 102L32 108Z" fill="url(#hair)"/><path d="M37 58Q41 89 39 101M83 58Q79 90 81 101" fill="none" stroke="#fff" stroke-opacity=".16" stroke-width="3" stroke-linecap="round"/>',
 cacheado:'<g fill="url(#hair)"><circle cx="39" cy="39" r="10"/><circle cx="47" cy="25" r="11"/><circle cx="59" cy="20" r="13"/><circle cx="72" cy="25" r="11"/><circle cx="81" cy="39" r="10"/><circle cx="43" cy="51" r="10"/><circle cx="77" cy="51" r="10"/><circle cx="50" cy="35" r="9"/><circle cx="69" cy="35" r="9"/></g><path d="M43 28Q59 14 75 27" fill="none" stroke="#fff" stroke-opacity=".14" stroke-width="3" stroke-linecap="round"/>',
 crespo:'<path d="M35 51C32 25 44 8 60 8C77 8 89 25 85 52C78 41 70 35 60 35C50 35 42 41 35 51Z" fill="url(#hair)"/><path d="M39 31L34 38M47 19L43 12M57 15L58 8M67 17L71 11M78 29L85 22M43 40L38 47M76 40L82 47" stroke="#fff" stroke-opacity=".15" stroke-width="3.5" stroke-linecap="round"/>',
 coque:'<circle cx="76" cy="13" r="12" fill="url(#hair)"/><path d="M35 50C34 27 47 12 60 12C74 12 86 27 85 50C78 40 69 34 60 34C51 34 42 40 35 50Z" fill="url(#hair)"/><path d="M43 26Q60 15 77 26" fill="none" stroke="#fff" stroke-opacity=".18" stroke-width="3" stroke-linecap="round"/>',
 raspado:'<path d="M38 43C38 24 48 13 60 13C72 13 82 24 82 43Q74 34 60 34Q46 34 38 43Z" fill="url(#hair)"/><path d="M43 25Q60 15 77 25" fill="none" stroke="#fff" stroke-opacity=".2" stroke-width="2.5" stroke-linecap="round"/>',
 moicano:'<path d="M46 41L51 8L58 22L63 4L68 22L76 9L76 41Q69 34 60 34Q51 34 46 41Z" fill="url(#hair)"/><path d="M54 18L60 8L66 19" fill="none" stroke="#fff" stroke-opacity=".2" stroke-width="2.5" stroke-linecap="round"/>',
 franja:'<path d="M35 48C34 25 46 10 60 10C76 10 88 25 86 49C79 40 72 34 60 34C49 34 41 40 35 48Z" fill="url(#hair)"/><path d="M39 42Q48 25 59 29Q69 18 82 38L80 50Q72 39 60 38Q48 37 39 50Z" fill="url(#hair)"/><path d="M43 28Q58 15 77 27" fill="none" stroke="#fff" stroke-opacity=".18" stroke-width="3" stroke-linecap="round"/>',
 lateral:'<path d="M34 50C31 26 44 9 60 9C78 9 90 26 86 55L80 88L72 52C69 40 65 34 60 34C54 34 49 41 46 53L39 88Z" fill="url(#hair)"/><path d="M76 35Q84 48 80 76" fill="none" stroke="#fff" stroke-opacity=".16" stroke-width="3" stroke-linecap="round"/><path d="M43 28Q59 14 76 27" fill="none" stroke="#fff" stroke-opacity=".14" stroke-width="2.5" stroke-linecap="round"/>'
 }[hair]||"";
 const eyes={normal:'<g><ellipse cx="51" cy="52" rx="3.7" ry="4.2" fill="#172033"/><ellipse cx="69" cy="52" rx="3.7" ry="4.2" fill="#172033"/><circle cx="49.8" cy="50.6" r="1.15" fill="#fff"/><circle cx="67.8" cy="50.6" r="1.15" fill="#fff"/></g>',grande:'<g><ellipse cx="51" cy="52" rx="4.8" ry="5.5" fill="#172033"/><ellipse cx="69" cy="52" rx="4.8" ry="5.5" fill="#172033"/><circle cx="49.6" cy="50.2" r="1.3" fill="#fff"/><circle cx="67.6" cy="50.2" r="1.3" fill="#fff"/></g>',fechado:'<path d="M47 52Q51 48 55 52M65 52Q69 48 73 52" fill="none" stroke="#172033" stroke-width="2.4" stroke-linecap="round"/>',estreito:'<g><ellipse cx="51" cy="52" rx="3.2" ry="2.5" fill="#172033"/><ellipse cx="69" cy="52" rx="3.2" ry="2.5" fill="#172033"/><circle cx="50" cy="51.2" r=".9" fill="#fff"/><circle cx="68" cy="51.2" r=".9" fill="#fff"/></g>',brilhante:'<g><ellipse cx="51" cy="52" rx="4.2" ry="4.8" fill="#172033"/><ellipse cx="69" cy="52" rx="4.2" ry="4.8" fill="#172033"/><circle cx="49.7" cy="50.4" r="1.35" fill="#fff"/><circle cx="67.7" cy="50.4" r="1.35" fill="#fff"/><circle cx="52.3" cy="53.7" r=".65" fill="#fff"/><circle cx="70.3" cy="53.7" r=".65" fill="#fff"/></g>'
 }[eye]||"";
 const brows={normal:'<path d="M47 44Q51 42 55 44M65 44Q69 42 73 44" fill="none" stroke="#35261f" stroke-width="2.5" stroke-linecap="round"/>',reto:'<path d="M47 43H55M65 43H73" fill="none" stroke="#35261f" stroke-width="2.7" stroke-linecap="round"/>',arqueada:'<path d="M47 45Q51 39 55 44M65 44Q69 39 73 45" fill="none" stroke="#35261f" stroke-width="2.7" stroke-linecap="round"/>',forte:'<path d="M46 44Q51 39 56 43M64 43Q69 39 74 44" fill="none" stroke="#35261f" stroke-width="3.5" stroke-linecap="round"/>',preocupada:'<path d="M47 46Q51 41 55 44M65 44Q69 41 73 46" fill="none" stroke="#35261f" stroke-width="2.7" stroke-linecap="round"/>'
 }[brow]||"";
 const mouths={normal:'<path d="M57 67Q60 69 63 67" fill="none" stroke="#88483f" stroke-width="2" stroke-linecap="round"/>',sorriso:'<path d="M54 65Q60 73 66 65Q60 69 54 65Z" fill="#8b4940"/><path d="M56 66Q60 68 64 66" fill="none" stroke="#fff" stroke-width="1.2"/>',serio:'<path d="M55 67H65" fill="none" stroke="#88483f" stroke-width="2.2" stroke-linecap="round"/>',aberta:'<ellipse cx="60" cy="67" rx="5" ry="3.8" fill="#6d3735"/><path d="M57 66H63" stroke="#fff" stroke-width="1.1"/>',surpreso:'<ellipse cx="60" cy="67" rx="3.7" ry="4.8" fill="#6d3735"/><ellipse cx="60" cy="66" rx="1.5" ry="1" fill="#fff"/>',triste:'<path d="M55 69Q60 65 65 69" fill="none" stroke="#88483f" stroke-width="2.1" stroke-linecap="round"/>'
 }[mouth]||"";
 const noses={normal:'<path d="M60 53L58 60Q60 61 62 60" fill="none" stroke="#9a5548" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>',fino:'<path d="M60 53L59 60Q60 61 61 60" fill="none" stroke="#9a5548" stroke-width="1.1" stroke-linecap="round"/>',arredondado:'<path d="M60 53L57 60Q60 63 63 60" fill="none" stroke="#9a5548" stroke-width="1.4" stroke-linecap="round"/>',arrebitado:'<path d="M60 53L58 60Q60 58 62 60" fill="none" stroke="#9a5548" stroke-width="1.3" stroke-linecap="round"/>'
 }[nose]||"";
 const hairCrown=(hair==='raspado'||hair==='moicano')?'':`<path d="M36 31C35 18 45 7 60 7C75 7 85 18 84 31Q77 24 69 22Q60 19 51 22Q43 24 36 31Z" fill="url(#hair)"/><path d="M42 22Q60 10 78 22" fill="none" stroke="#fff" stroke-opacity=".14" stroke-width="2.4" stroke-linecap="round"/>`;
 const hairShine='<path d="M43 24Q60 10 77 24" fill="none" stroke="#fff" stroke-opacity=".11" stroke-width="1.8" stroke-linecap="round"/><path d="M38 36Q40 27 46 21" fill="none" stroke="#fff" stroke-opacity=".08" stroke-width="2" stroke-linecap="round"/>';
 const hairFront=hair==='franja'?'<path d="M38 40C42 28 50 22 60 22C70 22 78 28 82 40Q74 33 67 34Q60 31 53 34Q46 33 38 40Z" fill="url(#hair)"/><path d="M43 31Q60 21 77 31" fill="none" stroke="#fff" stroke-opacity=".16" stroke-width="2.2" stroke-linecap="round"/>':""; const genderDetails=female?'<path d="M48 76Q60 81 72 76" fill="none" stroke="#6f3f34" stroke-opacity=".18" stroke-width="1.5"/><path d="M47 52L45 50M73 52L75 50" stroke="#35261f" stroke-width="1.1" stroke-linecap="round"/>':'';
 const folds=body==="magro"?2:body==="forte"?4:3;
 const foldPaths=Array.from({length:folds},(_,i)=>'<path d="M'+(45+i*8)+' 92Q60 '+(96+(i%2)*3)+' '+(75-i*8)+' 92" fill="none" stroke="#fff" stroke-opacity=".18" stroke-width="1.5"/>').join("");
 const accessoryMeta=Array.isArray(window.charAccessoryMeta)?window.charAccessoryMeta:[];
 const equipped=(x.accessories||[]).map(id=>accessoryMeta.find(a=>String(a.id)===String(id))).filter(Boolean).slice(0,4);
 const positions=[{x:60,y:18,rot:0},{x:60,y:91,rot:0},{x:88,y:108,rot:-8},{x:91,y:76,rot:8}];
 const acc=equipped.map((a,i)=>{const p=positions[i],emoji=esc(a.emoji||"🎒");return '<g class="char-item" transform="translate('+p.x+' '+p.y+') rotate('+p.rot+')"><circle r="13" fill="#0b1220" fill-opacity=".94" stroke="#fff" stroke-opacity=".32"/><text text-anchor="middle" dominant-baseline="central" font-size="13">'+emoji+'</text></g>'}).join("");
...
async function accountPage(box){
 ...
 charAccessoryMeta=owned.map((x,i)=>({id:x.product.id,emoji:x.product.emoji||"🎒",name:x.product.name||"Acessório",position:x.product.position||x.product.slot||["head","chest","wrist","side"][i%4]}));
 window.charAccessoryMeta=charAccessoryMeta;
 const skinPalette=['#f6d2b5','#f1c27d','#d9a066','#ae6b45','#7a4a2d','#4b2e1f'];
 const hairPalette=['#15110e','#2b2118','#4b3024','#6b442d','#9a6a3f','#d8b27c','#eee1c7','#6b6f78'];
 const skinDots=skinPalette.map(v=>'<button class="color-swatch" style="background:'+v+'" onclick="setCharacterField(\'skin\',\''+v+'\')"></button>').join('');
 const hairDots=hairPalette.map(v=>'<button class="color-swatch" style="background:'+v+'" onclick="setCharacterField(\'hair\',\''+v+'\')"></button>').join('');
 const hairStyles=['curto','medio','longo','cacheado','crespo','coque','raspado','moicano','franja','lateral'];
 const hairButtons=hairStyles.map(h=>'<button class="hair-choice '+(c.hairStyle===h?'selected':'')+'" data-choice-key="hairStyle" data-choice-value="'+h+'" onclick="setCharacterField(\'hairStyle\',\''+h+'\')">'+h[0].toUpperCase()+h.slice(1)+'</button>').join('');
 const accessoriesHtml=owned.length?owned.map(x=>'<button class="owned-accessory '+((c.accessories||[]).includes(x.product.id)?'equipped':'')+'" onclick="toggleOwnedAccessory(\''+x.product.id+'\')"><span>'+esc(x.product.emoji||'🎒')+'</span><b>'+esc(x.product.name)+'</b><small>'+x.quantity+'x</small></button>').join(''):'<div class="empty-accessories">Você ainda não comprou acessórios. Compre produtos nas Lojas para equipá-los aqui.</div>';
 box.innerHTML='<div class="character-profile">'+
  ...
  '<div class="character-editor">'+
   ...
   '<div class="char-tabs">'+
    ...
    '<button class="char-tab" onclick="showCharacterTab(\'body\')">Corpo & Pele</button>'+
    '<button class="char-tab" onclick="showCharacterTab(\'hair\')">Cabelo</button>'+
    '<button class="char-tab" onclick="showCharacterTab(\'ears\')">Orelhas</button>'+

    '<button class="char-tab" onclick="showCharacterTab(\'accessories\')">Acessórios <b>'+owned.length+'</b></button></div>'+
   '<div id="charFace" class="char-panel"><div class="choice-title">Gênero</div><div class="choice-row">'+
    '<button class="choice-btn '+(c.gender!=="feminino"?'selected':'')+'" data-choice-key="gender" data-choice-value="masculino" onclick="setCharacterField(\'gender\',\'masculino\')">♂ Masculino</button>'+
    '<button class="choice-btn '+(c.gender==="feminino"?'selected':'')+'" data-choice-key="gender" data-choice-value="feminino" onclick="setCharacterField(\'gender\',\'feminino\')">♀ Feminino</button></div>'+
    '<div class="face-editor"><div class="choice-title">Olhos</div><div class="choice-row">'+
    '<button class="choice-btn '+(c.eyeStyle==='normal'?'selected':'')+'" data-choice-key="eyeStyle" data-choice-value="normal" onclick="setCharacterField(\'eyeStyle\',\'normal\')">Normais</button><button class="choice-btn '+(c.eyeStyle==='grande'?'selected':'')+'" data-choice-key="eyeStyle" data-choice-value="grande" onclick="setCharacterField(\'eyeStyle\',\'grande\')">Grandes</button><button class="choice-btn '+(c.eyeStyle==='fechado'?'selected':'')+'" data-choice-key="eyeStyle" data-choice-value="fechado" onclick="setCharacterField(\'eyeStyle\',\'fechado\')">Fechados</button><button class="choice-btn '+(c.eyeStyle==='estreito'?'selected':'')+'" data-choice-key="eyeStyle" data-choice-value="estreito" onclick="setCharacterField(\'eyeStyle\',\'estreito\')">Estreitos</button><button class="choice-btn '+(c.eyeStyle==='brilhante'?'selected':'')+'" data-choice-key="eyeStyle" data-choice-value="brilhante" onclick="setCharacterField(\'eyeStyle\',\'brilhante\')">Brilhantes</button></div>'+
    '<div class="choice-title">Sobrancelhas</div><div class="choice-row"><button class="choice-btn '+(c.browStyle==='normal'?'selected':'')+'" data-choice-key="browStyle" data-choice-value="normal" onclick="setCharacterField(\'browStyle\',\'normal\')">Normais</button><button class="choice-btn '+(c.browStyle==='reto'?'selected':'')+'" data-choice-key="browStyle" data-choice-value="reto" onclick="setCharacterField(\'browStyle\',\'reto\')">Retas</button><button class="choice-btn '+(c.browStyle==='arqueada'?'selected':'')+'" data-choice-key="browStyle" data-choice-value="arqueada" onclick="setCharacterField(\'browStyle\',\'arqueada\')">Arqueadas</button><button class="choice-btn '+(c.browStyle==='forte'?'selected':'')+'" data-choice-key="browStyle" data-choice-value="forte" onclick="setCharacterField(\'browStyle\',\'forte\')">Fortes</button><button class="choice-btn '+(c.browStyle==='preocupada'?'selected':'')+'" data-choice-key="browStyle" data-choice-value="preocupada" onclick="setCharacterField(\'browStyle\',\'preocupada\')">Preocupadas</button></div>'+
    '<div class="choice-title">Nariz</div><div class="choice-row"><button class="choice-btn '+(c.noseStyle==='normal'?'selected':'')+'" data-choice-key="noseStyle" data-choice-value="normal" onclick="setCharacterField(\'noseStyle\',\'normal\')">Normal</button><button class="choice-btn '+(c.noseStyle==='fino'?'selected':'')+'" data-choice-key="noseStyle" data-choice-value="fino" onclick="setCharacterField(\'noseStyle\',\'fino\')">Fino</button><button class="choice-btn '+(c.noseStyle==='arredondado'?'selected':'')+'" data-choice-key="noseStyle" data-choice-value="arredondado" onclick="setCharacterField(\'noseStyle\',\'arredondado\')">Arredondado</button><button class="choice-btn '+(c.noseStyle==='arrebitado'?'selected':'')+'" data-choice-key="noseStyle" data-choice-value="arrebitado" onclick="setCharacterField(\'noseStyle\',\'arrebitado\')">Arrebitado</button></div>'+
    '<div class="choice-title">Boca / expressão</div><div class="choice-row"><button class="choice-btn '+(c.mouthStyle==='normal'?'selected':'')+'" data-choice-key="mouthStyle" data-choice-value="normal" onclick="setCharacterField(\'mouthStyle\',\'normal\')">Normal</button><button class="choice-btn '+(c.mouthStyle==='sorriso'?'selected':'')+'" data-choice-key="mouthStyle" data-choice-value="sorriso" onclick="setCharacterField(\'mouthStyle\',\'sorriso\')">Sorriso</button><button class="choice-btn '+(c.mouthStyle==='serio'?'selected':'')+'" data-choice-key="mouthStyle" data-choice-value="serio" onclick="setCharacterField(\'mouthStyle\',\'serio\')">Sério</button><button class="choice-btn '+(c.mouthStyle==='aberta'?'selected':'')+'" data-choice-key="mouthStyle" data-choice-value="aberta" onclick="setCharacterField(\'mouthStyle\',\'aberta\')">Aberta</button><button class="choice-btn '+(c.mouthStyle==='surpreso'?'selected':'')+'" data-choice-key="mouthStyle" data-choice-value="surpreso" onclick="setCharacterField(\'mouthStyle\',\'surpreso\')">Surpreso</button><button class="choice-btn '+(c.mouthStyle==='triste'?'selected':'')+'" data-choice-key="mouthStyle" data-choice-value="triste" onclick="setCharacterField(\'mouthStyle\',\'triste\')">Triste</button></div></div></div>'+
   '<div id="charBody" class="char-panel hidden"><div class="choice-title">Tipo de corpo</div><div class="choice-row"><button class="choice-btn '+(c.bodyType==='magro'?'selected':'')+'" data-choice-key="bodyType" data-choice-value="magro" onclick="setCharacterField(\'bodyType\',\'magro\')">Magro</button><button class="choice-btn '+(c.bodyType==='normal'?'selected':'')+'" data-choice-key="bodyType" data-choice-value="normal" onclick="setCharacterField(\'bodyType\',\'normal\')">Normal</button><button class="choice-btn '+(c.bodyType==='forte'?'selected':'')+'" data-choice-key="bodyType" data-choice-value="forte" onclick="setCharacterField(\'bodyType\',\'forte\')">Forte</button></div><div class="choice-title">Tom de pele</div><div class="quick-palette">'+skinDots+'</div><div class="color-grid"><label>Pele<input id="charSkin" type="color" value="'+(c.skin||'#f1c27d')+'" oninput="setCharacterField(\'skin\',this.value)"></label></div></div>'+
   '<div id="charHair" class="char-panel hidden"><div class="choice-title">Estilo de cabelo</div><div class="hair-grid">'+hairButtons+'</div><div class="choice-title">Cor do cabelo</div><div class="quick-palette">'+hairDots+'</div><div class="color-grid"><label>Cabelo<input id="charHair" type="color" value="'+(c.hair||'#2b2118')+'" oninput="setCharacterField(\'hair\',this.value)"></label></div></div>'+
   '<div id="charEars" class="char-panel hidden"><div class="choice-title">Formato das orelhas</div><div class="choice-row">'+
    '<button class="choice-btn '+(c.earStyle==="normal"?"selected":"")+'" data-choice-key="earStyle" data-choice-value="normal" onclick="setCharacterField(\'earStyle\',\'normal\')">Normais</button>'+
    '<button class="choice-btn '+(c.earStyle==="pequena"?"selected":"")+'" data-choice-key="earStyle" data-choice-value="pequena" onclick="setCharacterField(\'earStyle\',\'pequena\')">Pequenas</button>'+
    '<button class="choice-btn '+(c.earStyle==="redonda"?"selected":"")+'" data-choice-key="earStyle" data-choice-value="redonda" onclick="setCharacterField(\'earStyle\',\'redonda\')">Redondas</button>'+
    '<button class="choice-btn '+(c.earStyle==="pontuda"?"selected":"")+'" data-choice-key="earStyle" data-choice-value="pontuda" onclick="setCharacterField(\'earStyle\',\'pontuda\')">Pontudas</button></div></div>'+
   '<div id="charAccessories" class="char-panel hidden"><div class="owned-accessories">'+accessoriesHtml+'</div></div>'+
...
function getCharacterDraft(){
 const c=me.character||{};
 return {gender:c.gender||'masculino',skin:$('#charSkin')?.value||c.skin||'#f1c27d',hair:$('#charHair')?.value||c.hair||'#2b2118',hairStyle:c.hairStyle||'curto',shirt:$('#charShirt')?.value||c.shirt||'#4f6cff',pants:$('#charPants')?.value||c.pants||'#273449',shoes:$('#charShoes')?.value||c.shoes||'#151a22',bodyType:c.bodyType||'normal',eyeStyle:c.eyeStyle||'normal',browStyle:c.browStyle||'normal',mouthStyle:c.mouthStyle||'normal',earStyle:c.earStyle||'normal',noseStyle:c.noseStyle||'normal',accessories:Array.isArray(c.accessories)?c.accessories.slice():[],held:c.held||null};
}
function setCharacterField(k,v){
 if(v===undefined||v===null||v==="")return;
 const c=getCharacterDraft();c[k]=v;me.character=c;renderCharacterPreview();
 $('[data-choice-key="'+k+'"]').forEach(b=>b.classList.toggle('selected',b.dataset.choiceValue===String(v)));
}
function showCharacterTab(tab){
 const map={face:'charFace',body:'charBody',hair:'charHair',ears:'charEars',accessories:'charAccessories'};
 Object.keys(map).forEach(k=>$('#'+map[k])?.classList.toggle('hidden',k!==tab));
 $$('.char-tab').forEach(b=>b.classList.toggle('active',b.getAttribute('onclick')?.includes("'"+tab+"'")));
}
function toggleOwnedAccessory(id){
 const c=getCharacterDraft(),a=new Set(c.accessories);a.has(id)?a.delete(id):a.add(id);c.accessories=[...a];me.character=c;renderCharacterPreview();showCharacterTab('accessories');
}
