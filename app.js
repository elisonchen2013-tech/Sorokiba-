...
function technologyCustomizationFields(v={}){
 const x=v||{};
 return '<div class="product-customizer"><strong>💻 Personalização da tecnologia</strong><div class="form-row-2"><label>Categoria<input name="techCategory" maxlength="40" value="'+esc(x.category||'Eletrônico')+'"></label><label>Marca<input name="techBrand" maxlength="50" value="'+esc(x.brand||'')+'"></label><label>Versão<input name="techVersion" maxlength="30" value="'+esc(x.version||'1.0')+'"></label><label>Cor<input name="techColor" type="color" value="'+(x.color||'#5b6cff')+'"></label></div><label>Especificações<textarea name="techSpecs" maxlength="180">'+esc(x.specs||'')+'</textarea></label></div>';
}
function readTechnologyCustomization(f){return{category:f.techCategory?.value,brand:f.techBrand?.value,version:f.techVersion?.value,specs:f.techSpecs?.value,color:f.techColor?.value}}
function clothingCustomizationFields(v={}){
 const x=v||{};
 return '<div class="product-customizer"><strong>👕 Personalização da roupa</strong><div class="form-row-2"><label>Categoria<select name="clothCategory"><option '+(x.category==='Camiseta'?'selected':'')+'>Camiseta</option><option '+(x.category==='Calça'?'selected':'')+'>Calça</option><option '+(x.category==='Jaqueta'?'selected':'')+'>Jaqueta</option><option '+(x.category==='Boné'?'selected':'')+'>Boné</option><option '+(x.category==='Tênis'?'selected':'')+'>Tênis</option><option '+(x.category==='Acessório'?'selected':'')+'>Acessório</option></select></label><label>Estilo<input name="clothStyle" maxlength="40" value="'+esc(x.style||'Casual')+'"></label><label>Tamanho<select name="clothSize"><option '+(x.size==='P'?'selected':'')+'>P</option><option '+(x.size==='M'||!x.size?'selected':'')+'>M</option><option '+(x.size==='G'?'selected':'')+'>G</option><option '+(x.size==='GG'?'selected':'')+'>GG</option></select></label><label>Estampa<input name="clothPattern" maxlength="40" value="'+esc(x.pattern||'Lisa')+'"></label><label>Cor principal<input name="clothPrimaryColor" type="color" value="'+(x.primaryColor||'#222222')+'"></label><label>Cor secundária<input name="clothSecondaryColor" type="color" value="'+(x.secondaryColor||'#eeeeee')+'"></label></div><label>Logo/marca<input name="clothLogo" maxlength="50" value="'+esc(x.logo||'')+'"></label></div>';
}
...
function productCustomizationForForm(f){
 const box=f.querySelector("#productCustomizationBox");if(!box)return;
 const type=f.type?.value;
 if(type==="tecnologia")box.innerHTML=technologyCustomizationFields();
 else if(type==="roupa")box.innerHTML=clothingCustomizationFields();
 else box.innerHTML="";
}
