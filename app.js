...
function characterSvg(c){
 const x=c||{}, female=x.gender==="feminino", hair=x.hairStyle||"curto", ear=x.earStyle||"normal";
 ...
 const earShape=ear==="pequena"?'<ellipse cx="35" cy="48" rx="3.8" ry="5" fill="'+skin+'"/><ellipse cx="85" cy="48" rx="3.8" ry="5" fill="'+skin+'"/>':ear==="redonda"?'<circle cx="35" cy="48" r="5.5" fill="'+skin+'"/><circle cx="85" cy="48" r="5.5" fill="'+skin+'"/>':ear==="pontuda"?'<path d="M36 43L27 48Q31 55 37 53Z" fill="'+skin+'"/><path d="M84 43L93 48Q89 55 83 53Z" fill="'+skin+'"/>':'<ellipse cx="35" cy="48" rx="5" ry="6" fill="'+skin+'"/><ellipse cx="85" cy="48" rx="5" ry="6" fill="'+skin+'"/>';
 const body=female
 ...
 return `<svg viewBox="0 0 120 190" class="character-svg" aria-label="Personagem ${female?'feminino':'masculino'}">
  <g class="char-body">
   ...
   <path d="M36 49C35 25 45 7 60 7C75 7 85 25 84 49C78 39 70 34 60 34C50 34 42 39 36 49Z" fill="${hairColor}"/>
...
function getCharacterDraft(){
 const c=me.character||{};
 return {gender:c.gender||'masculino',earStyle:c.earStyle||'normal',skin:$('#charSkin')?.value||c.skin,hair:$('#charHair')?.value||c.hair,hairStyle:c.hairStyle||'curto',shirt:$('#charShirt')?.value||c.shirt,pants:$('#charPants')?.value||c.pants,shoes:$('#charShoes')?.value||c.shoes,accessories:Array.isArray(c.accessories)?c.accessories.slice():[],held:c.held||null};
}
