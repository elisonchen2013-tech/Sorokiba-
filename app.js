...
async function openCompanyDashboard(id){
 ...
 const sales=d.sales||[];
 openModal(`<div class="company-dashboard"><span class="eyebrow">PAINEL DA EMPRESA</span><h2>${esc(c.name)}</h2><div class="company-kpis dashboard-kpis"><div><b>${money(d.balance)}</b><small>Faturamento acumulado</small></div><div><b>${d.salesCount||0}</b><small>Produtos vendidos</small></div><div><b>Nível ${d.level||1}</b><small>${d.xp||0} XP da empresa</small></div><div><b>R$ 250</b><small>Taxa semanal ao prefeito</small></div></div><div class="company-dashboard-title"><h3>Produtos</h3><button class="primary" onclick="openAddCompanyProduct('${id}')">＋ Produto</button></div><div class="company-products-grid">${(c.products||[]).map(p=>`<div class="owner-product-mini"><b>${esc(p.name)}</b><span>${money(p.price)}</span><small>${esc(p.type)}</small></div>`).join('')}</div><div class="company-dashboard-title"><h3>Últimas vendas</h3></div><div class="sales-list">${sales.length?sales.slice(0,20).map(s=>`<div><b>${esc(s.product)}</b><span>x${s.quantity} — ${money(s.total)}</span><small>${new Date(s.date).toLocaleDateString('pt-BR')}</small></div>`).join(''):'<div class="empty">Nenhuma venda ainda.</div>'}</div></div>`);
}
async function shopPage(box){
 const d=await api("/api/companies"),featured=d.featured||[],recent=d.recent||[];
 const card=c=>`<article class="company-card" onclick="openCompany('${c.id}')"><div class="company-cover">${c.products?.[0]?.image?'<img src="'+c.products[0].image+'" alt="">':'<span>🏢</span>'}</div><div class="company-card-body"><div class="company-name-row"><h3>${esc(c.name)}</h3>${c.featured?'<span class="company-featured">DESTAQUE</span>':''}</div><p>${esc(c.description||'Empresa de Sorokiba')}</p><small>🛍️ ${c.productCount} produto${c.productCount===1?'':'s'}</small></div></article>`;
 box.innerHTML=`<div class="page-intro"><div><span class="eyebrow">LOJAS DE SOROKIBA</span><h1>Comprar</h1><p>Escolha uma empresa e compre seus produtos.</p></div></div><div class="company-search"><input placeholder="Pesquisar empresa ou produto..." oninput="searchCompanies(this.value)"></div><div id="companyStoreResults">${featured.length?`<section class="company-section"><div class="section-head"><div><span class="eyebrow">EM DESTAQUE</span><h3>Empresas em destaque</h3></div></div><div class="companies-grid">${featured.map(card).join('')}</div></section>`:''}<section class="company-section"><div class="section-head"><div><span class="eyebrow">RECENTES</span><h3>Empresas</h3></div></div><div class="companies-grid">${recent.length?recent.map(card).join(''):'<div class="empty"><div>🏪</div><h3>Ainda não há empresas</h3></div>'}</div></section></div>`;
}
async function searchCompanies(q){const d=await api("/api/companies?search="+encodeURIComponent(q||"")),box=document.getElementById("companyStoreResults");if(!box)return;const list=d.companies||[];const card=c=>`<article class="company-card" onclick="openCompany('${c.id}')"><div class="company-cover">${c.products?.[0]?.image?'<img src="'+c.products[0].image+'" alt="">':'<span>🏢</span>'}</div><div class="company-card-body"><h3>${esc(c.name)}</h3><p>${esc(c.description||'')}</p><small>🛍️ ${c.productCount} produtos</small></div></article>`;box.innerHTML=`<section class="company-section"><div class="section-head"><div><span class="eyebrow">RESULTADOS</span><h3>${list.length} empresa${list.length===1?'':'s'}</h3></div></div><div class="companies-grid">${list.length?list.map(card).join(''):'<div class="empty"><div>🔎</div><h3>Nenhuma empresa encontrada</h3></div>'}</div></section>`;
}
async function openCompany(id){const d=await api("/api/companies/"+encodeURIComponent(id)),c=d.company,isOwner=d.isOwner;const products=(c.products||[]).map(p=>`<article class="company-product"><div class="product-photo">${p.image?'<img src="'+p.image+'" alt="">':'<span>📦</span>'}</div><div class="product-info"><span class="product-type">${esc(p.type||'produto')}</span><h3>${esc(p.name)}</h3><p>${esc(p.description||'Sem descrição.')}</p><strong>${money(p.price)}</strong><button class="primary" onclick="openCompanyBuy('${c.id}','${p.id}',${p.price},'${esc(p.name)}')" ${isOwner?'disabled':''}>Comprar</button></div></article>`).join('');openModal(`<div class="company-detail"><span class="eyebrow">EMPRESA</span><h2>${esc(c.name)}</h2><p>${esc(c.description||'')}</p><div class="company-owner">👤 ${esc(c.ownerName||c.ownerUsername)}</div><div class="company-products-title"><h3>Produtos</h3></div><div class="company-products-grid">${products||'<div class="empty">Nenhum produto.</div>'}</div></div>`)}
function openCompanyBuy(companyId,productId,price,name){openModal(`<h2>Comprar ${name}</h2><p>Preço: ${money(price)}</p><label>Quantidade:<input id="companyBuyQty" type="number" min="1" max="99" value="1"></label><button class="primary" onclick="confirmCompanyBuy('${companyId}','${productId}')">Confirmar compra</button><button class="ghost" onclick="closeModal()">Cancelar</button>`)}
async function confirmCompanyBuy(companyId,productId){try{const q=Math.max(1,Math.min(99,Math.floor(Number(document.getElementById("companyBuyQty").value)||1)));const d=await post("/api/companies/"+encodeURIComponent(companyId)+"/products/"+encodeURIComponent(productId)+"/buy",{quantity:q});me=d.user;updateHUD();closeModal();toast(d.message);openCompany(companyId)}catch(e){toast(e.message,"error")}}
...
async function searchCompanies(q){
 ...
 const list=d.companies||[],card=c=>`<article class="company-card" onclick="openCompany('${c.id}')"><div class="company-cover">${c.products?.[0]?.image?'<img src="'+c.products[0].image+'" alt="">':'<span>🏢</span>'}</div><div class="company-card-body"><h3>${esc(c.name)}</h3><p>${esc(c.description||'')}</p><small>🛍️ ${c.productCount} produto${c.productCount===1?'':'s'}</small></div></article>`;
 box.innerHTML=`<section class="company-section"><div class="section-head"><div><span class="eyebrow">RESULTADOS</span><h3>${list.length} encontrada${list.length===1?'':'s'}</h3></div></div><div class="companies-grid">${list.length?list.map(card).join(''):'<div class="empty"><div>🔎</div><h3>Nenhuma empresa encontrada</h3></div>'}</div></section>`;
}
async function openCompany(id){
 const d=await api("/api/companies/"+encodeURIComponent(id)),c=d.company,isOwner=d.isOwner;
 const products=(c.products||[]).map(p=>`<article class="company-product"><div class="product-photo">${p.image?'<img src="'+p.image+'" alt="">':'<span>📦</span>'}</div><div class="product-info"><span class="product-type">${esc(p.type||'produto')}</span><h3>${esc(p.name)}</h3><p>${esc(p.description||'Sem descrição.')}</p>${p.type==='consumivel'&&Object.keys(p.effects||{}).length?'<div class="product-effects">'+Object.entries(p.effects||{}).map(([k,v])=>`<span>${k==='hunger'?'🍽️':k==='hydration'?'💧':k==='energy'?'⚡':'❤️'} ${v>0?'+':''}${v}</span>`).join('')+'</div>':''}<strong>${money(p.price)}</strong><button class="primary" onclick="openCompanyBuy('${c.id}','${p.id}',${p.price},'${esc(p.name)}')" ${isOwner?'disabled':''}>Comprar</button>${isOwner?'<button class="ghost" onclick="deleteCompanyProduct(\''+c.id+'\',\''+p.id+'\')">Remover</button>':''}</div></article>`).join('');
 openModal(`<div class="company-detail"><span class="eyebrow">EMPRESA</span><h2>${esc(c.name)}</h2><p>${esc(c.description||'')}</p><div class="company-owner">👤 ${esc(c.ownerName||c.ownerUsername)}</div><div class="company-products-title"><h3>Produtos</h3>${isOwner?'<button class="primary" onclick="openAddCompanyProduct(\''+c.id+'\')">＋ Adicionar</button>':''}</div><div class="company-products-grid">${products||'<div class="empty">Nenhum produto.</div>'}</div></div>`);
}
function openCompanyBuy(companyId,productId,price,name){openModal(`<h2>Comprar ${name}</h2><p>Preço: ${money(price)}</p><label>Quantidade:<input id="companyBuyQty" type="number" min="1" max="99" value="1"></label><button class="primary" onclick="confirmCompanyBuy('${companyId}','${productId}')">Confirmar compra</button><button class="ghost" onclick="closeModal()">Cancelar</button>`)}
async function confirmCompanyBuy(companyId,productId){try{const q=Math.max(1,Math.min(99,Math.floor(Number(document.getElementById("companyBuyQty").value)||1)));const d=await post("/api/companies/"+encodeURIComponent(companyId)+"/products/"+encodeURIComponent(productId)+"/buy",{quantity:q});me=d.user;updateHUD();closeModal();toast(d.message);openCompany(companyId)}catch(e){toast(e.message,"error")}}
...
function companyProductFields(){return `<label>Nome do produto<input name="productName" maxlength="80" required></label><label>Descrição<textarea name="productDescription" maxlength="300"></textarea></label><div class="form-row-2"><label>Preço (R$)<input name="price" type="number" min="0.01" max="1000000" step="0.01" required></label><label>Tipo<select name="type" onchange="toggleCompanyEffects(this.value)"><option value="consumivel">Consumível</option><option value="equipamento">Equipamento</option><option value="tecnologia">Tecnologia</option><option value="veiculo">Veículo</option><option value="decoracao">Decoração</option></select></label></div><div id="companyEffectsBox" class="company-effects-form"><strong>Efeitos ao usar</strong><div class="form-row-2"><label>🍽️ Fome<input name="hunger" type="number" min="-100" max="100" value="0"></label><label>💧 Hidratação<input name="hydration" type="number" min="-100" max="100" value="0"></label><label>⚡ Energia<input name="energy" type="number" min="-100" max="100" value="0"></label><label>❤️ Vida<input name="life" type="number" min="-100" max="100" value="0"></label></div></div><label>Foto do produto<input name="image" type="file" accept="image/png,image/jpeg,image/webp,image/gif"></label>`}
function openCreateCompany(){
 openModal(`<h2>Criar empresa</h2><p>Crie a empresa e o primeiro produto.</p><form id="createCompanyForm" class="company-form"><label>Nome da empresa<input name="name" maxlength="80" required></label><label>Descrição<textarea name="description" maxlength="500" required></textarea></label><h3>Primeiro produto</h3>${companyProductFields()}<button class="primary wide" type="submit">Criar empresa</button></form>`);
 const f=document.getElementById("createCompanyForm");f.onsubmit=async e=>{e.preventDefault();try{const image=await readProductImage(f.image);if(image===null)return;const d=await post("/api/companies",{name:f.name.value,description:f.description.value,product:{name:f.productName.value,description:f.productDescription.value,price:f.price.value,type:f.type.value,image,effects:{hunger:f.hunger.value,hydration:f.hydration.value,energy:f.energy.value,life:f.life.value}}});closeModal();toast(d.message);loadPage("shop")}catch(err){toast(err.message,"error")}};
}
