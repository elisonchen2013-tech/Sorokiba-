...
function characterSvg(c){
 const x=c||{},female=x.gender==="feminino",hair=x.hairStyle||"curto",body=x.bodyType||"normal",eye=x.eyeStyle||"normal",brow=x.browStyle||"normal",mouth=x.mouthStyle||"normal",nose=x.noseStyle||"normal",ear=x.earStyle||"normal";
 const skin=x.skin||"#f1c27d",hc=x.hair||"#2b2118",shirt=x.shirt||"#4f6cff",pants=x.pants||"#273449",shoes=x.shoes||"#151a22";
 const shapes={magro:{shoulder:18,waist:17,arm:6},normal:{shoulder:22,waist:20,arm:7},forte:{shoulder:26,waist:23,arm:8}},baseBody=shapes[body]||shapes.normal,b={shoulder:Math.max(16,baseBody.shoulder-(female?3:0)),waist:Math.max(15,baseBody.waist-(female?1:0)),arm:baseBody.arm-(female?0.5:0)}; const facePath=female?'M39 47C39 29 48 18 60 18C72 18 81 29 81 48C81 63 73 73 60 78C47 73 39 63 39 47Z':'M37 47C37 28 47 18 60 18C74 18 83 29 83 48C83 63 75 74 60 78C45 74 37 63 37 47Z';
 const hairMap={
 ...
 const hairFront=(hair==='raspado')?'':'<path d="M38 40C39 27 48 19 60 19C72 19 81 27 82 40C75 33 69 30 60 30C51 30 45 33 38 40Z" fill="url(#hair)"/><path d="M43 31Q60 20 77 31" fill="none" stroke="#fff" stroke-opacity=".14" stroke-width="2.2" stroke-linecap="round"/>';
 const earShape=ear==="pequena"?'<ellipse cx="35.5" cy="53" rx="3.2" ry="4.5" fill="'+skin+'" stroke="'+skin+'" stroke-width="1.1"/><ellipse cx="84.5" cy="53" rx="3.2" ry="4.5" fill="'+skin+'" stroke="'+skin+'" stroke-width="1.1"/>':ear==="redonda"?'<circle cx="35.5" cy="53" r="5" fill="'+skin+'" stroke="'+skin+'" stroke-width="1.1"/><circle cx="84.5" cy="53" r="5" fill="'+skin+'" stroke="'+skin+'" stroke-width="1.1"/>':ear==="pontuda"?'<path d="M35 48Q28 52 35 59Q40 55 39 50Z" fill="'+skin+'" stroke="'+skin+'" stroke-width="1.1"/><path d="M85 48Q92 52 85 59Q80 55 81 50Z" fill="'+skin+'" stroke="'+skin+'" stroke-width="1.1"/>':'<ellipse cx="35.5" cy="53" rx="4" ry="5.5" fill="'+skin+'" stroke="'+skin+'" stroke-width="1.2"/><ellipse cx="84.5" cy="53" rx="4" ry="5.5" fill="'+skin+'" stroke="'+skin+'" stroke-width="1.2"/>';
 const genderDetails=female?'<path d="M48 76Q60 81 72 76" fill="none" stroke="#6f3f34" stroke-opacity=".18" stroke-width="1.5"/><path d="M47 52L45 50M73 52L75 50" stroke="#35261f" stroke-width="1.1" stroke-linecap="round"/>':'';
 ...
 box.innerHTML='<div class="character-profile">'+
  ...
  '<div class="character-editor">'+
   ...
   '<div class="char-tabs">'+
    ...
    '<button class="char-tab" onclick="showCharacterTab(\'clothes\')">Roupas</button>'+
   ...
   '<div id="charEars" class="char-panel hidden"><div class="choice-title">Formato das orelhas</div><div class="choice-row"><button class="choice-btn '+(c.earStyle==='normal'?'selected':'')+'" data-choice-key="earStyle" data-choice-value="normal" onclick="setCharacterField(\'earStyle\',\'normal\')">Normais</button><button class="choice-btn '+(c.earStyle==='pequena'?'selected':'')+'" data-choice-key="earStyle" data-choice-value="pequena" onclick="setCharacterField(\'earStyle\',\'pequena\')">Pequenas</button><button class="choice-btn '+(c.earStyle==='redonda'?'selected':'')+'" data-choice-key="earStyle" data-choice-value="redonda" onclick="setCharacterField(\'earStyle\',\'redonda\')">Redondas</button><button class="choice-btn '+(c.earStyle==='pontuda'?'selected':'')+'" data-choice-key="earStyle" data-choice-value="pontuda" onclick="setCharacterField(\'earStyle\',\'pontuda\')">Pontudas</button></div></div>'+'<div id="charBody" class="char-panel hidden"><div class="choice-title">Tipo de corpo</div><div class="choice-row"><button class="choice-btn '+(c.bodyType==='magro'?'selected':'')+'" data-choice-key="bodyType" data-choice-value="magro" onclick="setCharacterField(\'bodyType\',\'magro\')">Magro</button><button class="choice-btn '+(c.bodyType==='normal'?'selected':'')+'" data-choice-key="bodyType" data-choice-value="normal" onclick="setCharacterField(\'bodyType\',\'normal\')">Normal</button><button class="choice-btn '+(c.bodyType==='forte'?'selected':'')+'" data-choice-key="bodyType" data-choice-value="forte" onclick="setCharacterField(\'bodyType\',\'forte\')">Forte</button></div><div class="choice-title">Tom de pele</div><div class="quick-palette">'+skinDots+'</div><div class="color-grid"><label>Pele<input id="charSkin" type="color" value="'+(c.skin||'#f1c27d')+'" oninput="setCharacterField(\'skin\',this.value)"></label></div></div>'+
...
function getCharacterDraft(){
 const c=me.character||{};
 return {gender:c.gender||'masculino',earStyle:c.earStyle||'normal',skin:$('#charSkin')?.value||c.skin||'#f1c27d',hair:$('#charHair')?.value||c.hair||'#2b2118',hairStyle:c.hairStyle||'curto',shirt:$('#charShirt')?.value||c.shirt||'#4f6cff',pants:$('#charPants')?.value||c.pants||'#273449',shoes:$('#charShoes')?.value||c.shoes||'#151a22',bodyType:c.bodyType||'normal',eyeStyle:c.eyeStyle||'normal',browStyle:c.browStyle||'normal',mouthStyle:c.mouthStyle||'normal',noseStyle:c.noseStyle||'normal',accessories:Array.isArray(c.accessories)?c.accessories.slice():[],held:c.held||null};
}
...
function showCharacterTab(tab){
 const map={face:'charFace',ears:'charEars',body:'charBody',hair:'charHair',clothes:'charClothes'};
