...
async function shopPage(box){
 ...
 const card=c=>`<article class="company-card" onclick="openCompany('${c.id}')"><div class="company-cover">${c.products?.[0]?.image?'<img src="'+c.products[0].image+'" alt="">':'<span>'+esc(c.products?.[0]?.emoji||'🏢')+'</span>'}</div><div class="company-card-body"><div class="company-name-row"><h3>${esc(c.name)}</h3>${c.featured?'<span class="company-featured">DESTAQUE</span>':''}</div><p>${esc(c.description||'Empresa de Sorokiba')}</p><small>🛍️ ${c.productCount} produto${c.productCount===1?'':'s'}</small></div></article>`;
...
async function searchCompanies(q){
 ...
 const list=d.companies||[],card=c=>`<article class="company-card" onclick="openCompany('${c.id}')"><div class="company-cover">${c.products?.[0]?.image?'<img src="'+c.products[0].image+'" alt="">':'<span>'+esc(c.products?.[0]?.emoji||'🏢')+'</span>'}</div><div class="company-card-body"><h3>${esc(c.name)}</h3><p>${esc(c.description||'')}</p><small>🛍️ ${c.productCount} produto${c.productCount===1?'':'s'}</small></div></article>`;
