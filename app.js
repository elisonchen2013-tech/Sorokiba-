...
async function shopPage(box){
 ...
 box.innerHTML=`<div class="page-intro company-store-head"><div><span class="eyebrow">LOJAS DE SOROKIBA</span><h1>Empresas e produtos</h1><p>Entre em uma empresa e compre produtos que podem ser usados na cidade.</p></div><button class="primary" onclick="openCreateCompany()">＋ Criar empresa</button></div><div class="company-search"><input placeholder="Pesquisar empresa ou produto..." oninput="searchCompanies(this.value)"></div><div id="companyStoreResults">${featured.length?`<section class="company-section"><div class="section-head"><div><span class="eyebrow">EM DESTAQUE</span><h3>Empresas em destaque</h3></div></div><div class="companies-grid">${featured.map(card).join('')}</div></section>`:''}<section class="company-section"><div class="section-head"><div><span class="eyebrow">RECENTES</span><h3>Novas empresas</h3></div></div><div class="companies-grid">${recent.length?recent.map(card).join(''):'<div class="empty"><div>🏪</div><h3>Ainda não há empresas</h3><p>Crie a primeira empresa.</p></div>'}</div></section></div>`;
}
