...
async function homeCarousel(box){
  ...
  const jobDecorations={estudante:['sc-job-student','📚','Sala de estudos'],entregador:['sc-job-delivery','📦','Central de entregas'],comerciante:['sc-job-shop','🛍️','Comércio local'],motorista:['sc-job-driver','🚕','Avenida de Sorokiba'],policial:['sc-job-police','🚓','Patrulha da cidade'],enfermeiro:['sc-job-nurse','🏥','Hospital'],medico:['sc-job-doctor','🩺','Clínica médica'],programador:['sc-job-programmer','💻','Centro de tecnologia'],engenheiro:['sc-job-engineer','🏗️','Canteiro de obras']};
  const selectedJobId=String(currentJob?.id||me?.jobId||me?.job?.id||'').toLowerCase();
