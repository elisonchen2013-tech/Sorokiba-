...
const startCooldownIfNeeded = (user) => {
  if (Number(user.missionBatchCount) >= 2) user.missionCooldownUntil = new Date(Date.now() + 3600 * 1000).toISOString();
};
...
// Recuperação de senha: não usa e-mail; exige usuário, nome e código de recuperação.
...
app.post('/api/missions/:id/answer',(req,res)=>{const {answer,questionIndex}=req.body;const mission=req.user.missions.find(m=>m.id===req.params.id);if(!mission)return res.status(400).json({error:'Missão não encontrada'});if(mission.status!=='active')return res.status(400).json({error:'Missão não está ativa'});const elapsed=(Date.now()-new Date(mission.started_at).getTime())/1000;if(elapsed>mission.duration_seconds){mission.status='expired';mission.completedAt=new Date().toISOString();registerMissionUse(req.user);saveData();return res.status(400).json({error:'Tempo esgotado para responder'})}if(typeof questionIndex!=='number'||questionIndex<0||questionIndex>=mission.questions.length)return res.status(400).json({error:'Índice de pergunta inválido'});const qid=mission.questionRefs?mission.questionRefs[questionIndex]:mission.questions[questionIndex].id;const q=findQuestionById(qid);const correctIndex=q?q.correct:null;const correct=answer===correctIndex;mission.answers=mission.answers||[];if(mission.answers[questionIndex])return res.status(400).json({error:'Pergunta já respondida'});mission.answers[questionIndex]={questionId:qid,selected:answer,correct};req.user.answeredQuestions=req.user.answeredQuestions||[];if(!req.user.answeredQuestions.includes(qid))req.user.answeredQuestions.push(qid);const total=mission.questions.length;const answeredCount=mission.answers.filter(Boolean).length;let xpGiven=0,moneyGiven=0,final=false;if(answeredCount===total){const correctCount=mission.answers.filter(a=>a&&a.correct).length;xpGiven=Math.floor((mission.rewardXp||0)*(correctCount/total));moneyGiven=correctCount===total?mission.rewardMoney||0:0;req.user.xp=(req.user.xp||0)+xpGiven;req.user.money=(req.user.money||0)+moneyGiven;while(req.user.xp>=500){req.user.level+=1;req.user.xp-=500}if(moneyGiven>0){req.user.transactions=req.user.transactions||[];req.user.transactions.push({date:new Date(),type:'mission',person:'Sistema',amount:moneyGiven,description:`Recompensa por missão (${mission.jobId})`})}mission.status='completed';mission.completedAt=new Date().toISOString();registerMissionUse(req.user);final=true;saveData()}else saveData();const correctOptionText=q?(q.options&&q.options[q.correct]):null;const state=getMissionState(req.user);res.json({message:correct?'Resposta correta!':'Resposta incorreta',correct,correctIndex,correctOptionText,xpGiven,moneyGiven,final,remaining:total-answeredCount,missionsRemaining:state.remaining,missionsUsed:state.batchCount,cooldownUntil:state.cooldownUntil,user:{name:req.user.name,username:req.user.username,money:req.user.money,level:req.user.level,xp:req.user.xp,jobName:req.user.jobName,life:req.user.life,hunger:req.user.hunger,hydration:req.user.hydration,energy:req.user.energy}})});
app.post('/api/missions/:id/complete',(req,res)=>{const mission=req.user.missions.find(m=>m.id===req.params.id);if(!mission)return res.status(400).json({error:'Missão não encontrada'});const elapsed=(Date.now()-new Date(mission.started_at).getTime())/1000;if(elapsed<mission.duration_seconds)return res.status(400).json({error:'Missão ainda não está completa'});if(mission.status==='active'){mission.status='expired';mission.completedAt=new Date().toISOString();registerMissionUse(req.user);saveData()}const state=getMissionState(req.user);res.json({message:'Missão finalizada',missionsRemaining:state.remaining,cooldownUntil:state.cooldownUntil,user:{name:req.user.name,username:req.user.username,money:req.user.money,level:req.user.level,xp:req.user.xp,jobName:req.user.jobName,life:req.user.life,hunger:req.user.hunger,hydration:req.user.hydration,energy:req.user.energy}})});

app.get('/api/inventory',(req,res)=>res.json({inventory:req.user.inventory||{},items:shopItems}));
app.post('/api/inventory/use',(req,res)=>{const {itemId}=req.body;const item=shopItems.find(i=>i.id===itemId);if(!item||!req.user.inventory[itemId])return res.status(400).json({error:'Item não encontrado no inventário'});if(item.hunger)req.user.hunger=Math.min(100,req.user.hunger+item.hunger);if(item.hydration)req.user.hydration=Math.min(100,req.user.hydration+item.hydration);if(item.energy)req.user.energy=Math.min(100,req.user.energy+item.energy);req.user.inventory[itemId]--;if(req.user.inventory[itemId]===0)delete req.user.inventory[itemId];saveData();res.json({message:`${item.name} usado com sucesso!`,user:{name:req.user.name,username:req.user.username,money:req.user.money,level:req.user.level,xp:req.user.xp,jobName:req.user.jobName,life:req.user.life,hunger:req.user.hunger,hydration:req.user.hydration,energy:req.user.energy}})});
app.get('/api/shop',(req,res)=>res.json(shopItems));
app.post('/api/shop/buy',(req,res)=>{const {itemId,quantity}=req.body;const item=shopItems.find(i=>i.id===itemId);if(!item)return res.status(400).json({error:'Item não encontrado'});const totalCost=item.price*quantity;if(req.user.money<totalCost)return res.status(400).json({error:'Dinheiro insuficiente'});req.user.money-=totalCost;req.user.inventory[itemId]=(req.user.inventory[itemId]||0)+quantity;req.user.transactions=req.user.transactions||[];req.user.transactions.push({date:new Date(),type:'purchase',person:'Loja',amount:totalCost,description:`${quantity}x ${item.name}`});saveData();res.json({message:`${quantity}x ${item.name} comprado!`,user:{name:req.user.name,username:req.user.username,money:req.user.money,level:req.user.level,xp:req.user.xp,jobName:req.user.jobName,life:req.user.life,hunger:req.user.hunger,hydration:req.user.hydration,energy:req.user.energy}})});
app.get('/api/hospital',(req,res)=>res.json({services:hospitalServices}));
app.post('/api/hospital/treat',(req,res)=>{const {serviceId}=req.body;const service=hospitalServices.find(s=>s.id===serviceId);if(!service)return res.status(400).json({error:'Serviço não encontrado'});if(req.user.money<service.price)return res.status(400).json({error:'Dinheiro insuficiente'});req.user.money-=service.price;req.user.life=Math.min(100,req.user.life+service.life);req.user.transactions=req.user.transactions||[];req.user.transactions.push({date:new Date(),type:'hospital',person:'Hospital',amount:service.price,description:service.name});saveData();res.json({message:`${service.name} realizado! Vida restaurada.`,user:{name:req.user.name,username:req.user.username,money:req.user.money,level:req.user.level,xp:req.user.xp,jobName:req.user.jobName,life:req.user.life,hunger:req.user.hunger,hydration:req.user.hydration,energy:req.user.energy}})});
...
app.post('/api/mayor/events',(req,res)=>{if(!req.user.isMayor)return res.status(403).json({error:'Apenas o prefeito pode criar eventos'});const {title,description,eventDate,image}=req.body;city.events.push({id:`event_${Date.now()}`,title,description,eventDate,image,createdAt:new Date()});saveData();res.json({message:'Evento criado!'})});
...
app.post('/api/mayor/rewards',(req,res)=>{if(!req.user.isMayor)return res.status(403).json({error:'Apenas o prefeito pode atualizar recompensas'});const {jobId,moneyPerMission,xpPerMission,questionsPerMission}=req.body;if(!jobId||!city.missionRewards[jobId])return res.status(400).json({error:'jobId inválido'});if(typeof moneyPerMission==='number')city.missionRewards[jobId].moneyPerMission=Math.max(10,Math.floor(moneyPerMission));if(typeof xpPerMission==='number')city.missionRewards[jobId].xpPerMission=Math.max(5,Math.floor(xpPerMission));if(typeof questionsPerMission==='number')city.missionRewards[jobId].questionsPerMission=Math.max(1,Math.min(5,Math.floor(questionsPerMission)));saveData();res.json({message:'Recompensas atualizadas',reward:city.missionRewards[jobId]})});

setInterval(()=>{Object.values(users).forEach(user=>{if(user.hunger>0)user.hunger-=1;if(user.hydration>0)user.hydration-=1;if(user.energy>0)user.energy-=1;if(user.life>0&&user.hunger<20)user.life-=2});saveData()},30000);

const PORT=process.env.PORT||3000;
(async()=>{await db.init();await loadData();if(!city.missionRewards)city.missionRewards={};jobs.forEach(j=>{if(!city.missionRewards[j.id])city.missionRewards[j.id]={moneyPerMission:Math.max(50,Math.floor(j.salary*.15)),xpPerMission:Math.max(20,Math.floor(j.salary*.08)),questionsPerMission:j.xpRequired>=1000?3:2}});saveData();app.listen(PORT,()=>console.log(`🏙️ Sorokiba rodando na porta ${PORT}`))})();
