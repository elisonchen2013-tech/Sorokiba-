...
const persistNow=()=>{
  saveQueue=saveQueue.then(async()=>{
    ...
    }catch(e){console.error('Falha ao salvar no Postgres',e);throw e;}
  }).catch(e=>{console.error('Falha ao salvar no Postgres',e)});
  return saveQueue;
};
const saveData=()=>{if(saveTimer)return;saveTimer=setTimeout(()=>{saveTimer=null;persistNow().catch(()=>{})},500)};
const saveDataNow=async()=>{if(saveTimer){clearTimeout(saveTimer);saveTimer=null}await persistNow()};
let tokenCounter=0;
...
const publicUser=(user)=>{if(!user)return null;const{password,recoveryCode,token,...safe}=user;return safe};
const ensureMissionRewards=()=>{
  if(!city.missionRewards||typeof city.missionRewards!=='object')city.missionRewards={};
  jobs.forEach(j=>{
...
const applyMissionRewards=(body={})=>{
  ensureMissionRewards();
  const payload=body.missionRewards&&typeof body.missionRewards==='object'&&!Array.isArray(body.missionRewards)?body.missionRewards:(body.jobId?{[body.jobId]:body}:{});
  for(const [jobId,value] of Object.entries(payload)){
    if(!value||typeof value!=='object')continue;
    city.missionRewards[jobId]={
      moneyPerMission:Math.max(0,Math.floor(Number(value.moneyPerMission)||0)),
      xpPerMission:Math.max(0,Math.floor(Number(value.xpPerMission)||0)),
      questionsPerMission:Math.max(1,Math.min(10,Math.floor(Number(value.questionsPerMission)||1)))
    };
  }
  return city.missionRewards;
};
const purgeAccount=(username)=>{
  const target=users[username];
  if(!target)return null;
  delete users[username];
  if(target.countedInPopulation)city.population=Math.max(0,Number(city.population||0)-1);
  if(Array.isArray(city.proposals)){
    city.proposals=city.proposals.filter(p=>{
      const owner=p?.authorUsername||p?.username||p?.createdBy;
      return owner!==username && p?.author!==target.name;
    });
  }
  return target;
};
...
app.post('/api/register',(req,res)=>{const{name,username,password,recoveryCode}=req.body;if(!name||!username||!password||!recoveryCode)return res.status(400).json({error:'Preencha todos os campos'});if(!USERNAME_RE.test(String(username)))return res.status(400).json({error:'Usuário inválido. Use 3 a 30 letras, números ou _'});if(String(password).length<6)return res.status(400).json({error:'A senha deve ter no mínimo 6 caracteres'});if(String(recoveryCode).length<6)return res.status(400).json({error:'O código de recuperação deve ter no mínimo 6 caracteres'});if(users[username])return res.status(400).json({error:'Usuário já existe'});const isMayor=Object.keys(users).length===0;users[username]=createUser(name,username,password,isMayor,recoveryCode);if(!users[username].countedInPopulation){city.population=Number(city.population||0)+1;users[username].countedInPopulation=true}users[username].token=generateToken();saveData();res.json({message:isMayor?'Conta criada! Você é o prefeito.':'Conta criada com sucesso!',token:users[username].token,user:publicUser(users[username])})});
app.post('/api/login',(req,res)=>{const{username,password}=req.body,user=users[username];if(!user||user.password!==password)return res.status(401).json({error:'Usuário ou senha incorretos'});user.token=generateToken();if(!user.countedInPopulation){city.population=Number(city.population||0)+1;user.countedInPopulation=true}saveData();res.json({message:'Login realizado!',token:user.token,user:publicUser(user)})});
...
app.post('/api/mayor/rewards',async(req,res)=>{
  try{
    if(!req.user.isMayor)return res.status(403).json({error:'Apenas o prefeito pode alterar recompensas'});
    const missionRewards=applyMissionRewards(req.body||{});
    await saveDataNow();
