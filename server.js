...
async function initDatabase() {
  ...
  /*
    ...
    Importante:
    id usa IDENTITY.
    Assim o PostgreSQL gera automaticamente o ID.
  */
  ...
  */
  ...
  /*
    ...
    user_id é INTEGER porque users.id é INTEGER.
  */
  ...
  */
  ...
  /*
    ...
    Se a tabela proposals já existia sem mayor_comment,
    adicionamos a coluna.

    Não usamos COALESCE misturando TEXT e INTEGER.
  */
  ...
  /*
    Garante que a sequência/IDENTITY de users continue funcionando
    mesmo se a tabela foi criada anteriormente.
  */
  ...
  try {
    ...
    console.log("Aviso ao ajustar sequência de users:", error.message);
  }
...
function calculateLevel(xp) {
  return Math.max(1, Math.floor(Number(xp) / 500) + 1);
}
...
async function updateNeeds(user) {
  ...
  const hours = Math.floor(
    Math.max(0, now - updatedAt) / 3600000
  );
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (!/^[a-zA-Z0-9_]{3,20}$/.test(username)) {
      return res.status(400).json({
        error: "Usuário deve ter de 3 a 20 caracteres."
      });
    }
    ...
    /*
      A primeira conta será prefeito.
    */
...
app.post("/api/job", auth, async (req, res) => {
  try {
    ...
    if (Number(req.user.xp) < requiredXP) {
      return res.status(400).json({
...
/*
  PREFEITO DECIDE A PROPOSTA

  Também pode deixar um comentário.
*/
