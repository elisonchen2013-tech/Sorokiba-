...
const vehicleCustomization=v=>{const s=v&&typeof v==='object'?v:{};const pick=(key,fallback)=>{const x=String(s[key]??fallback).trim();return /^#[0-9a-f]{6}$/i.test(x)?x:fallback};return{bodyColor:pick('bodyColor','#dfe6ee'),secondaryColor:pick('secondaryColor','#273449'),wheelColor:pick('wheelColor','#151a22'),windowColor:pick('windowColor','#7fc8e8'),neonColor:pick('neonColor','#7c5cff'),plateColor:pick('plateColor','#f2f2f2'),wheels:ct(s.wheels,30)||'Esportivas',windows:ct(s.windows,30)||'Originais',spoiler:!!s.spoiler,neon:!!s.neon,tint:Math.max(0,Math.min(100,Math.round(Number(s.tint)||0)))}};
const technologyCustomization=b=>{const x=b||{};const category=ct(x.category,40)||'Eletrônico';const brand=ct(x.brand,50);const version=ct(x.version,30)||'1.0';const specs=ct(x.specs,180);const color=typeof x.color==='string'&&/^#[0-9a-f]{6}$/i.test(x.color)?x.color:'#5b6cff';return{category,brand,version,specs,color}};
const clothingCustomization=b=>{const x=b||{};const category=ct(x.category,40)||'Camiseta';const style=ct(x.style,40)||'Casual';const size=ct(x.size,10)||'M';const primaryColor=typeof x.primaryColor==='string'&&/^#[0-9a-f]{6}$/i.test(x.primaryColor)?x.primaryColor:'#222222';const secondaryColor=typeof x.secondaryColor==='string'&&/^#[0-9a-f]{6}$/i.test(x.secondaryColor)?x.secondaryColor:'#eeeeee';const pattern=ct(x.pattern,40)||'Lisa';const logo=ct(x.logo,50);return{category,style,size,primaryColor,secondaryColor,pattern,logo}};
const productInput=b=>{const type=String(b?.type||'consumivel'),name=ct(b?.name,80),description=ct(b?.description,300),price=Number(b?.price),image=typeof b?.image==='string'?b.image.trim():'';const emoji=ct(b?.emoji,8)||'📦';if(!companyProductTypes[type])return{error:'Tipo de produto inválido.'};if(!name)return{error:'Informe o nome do produto.'};if(!Number.isFinite(price)||price<=0||price>1000000)return{error:'Preço inválido.'};if(image.length>550000)return{error:'A foto é grande demais.'};if(image&&!/^data:image\/(png|jpeg|jpg|webp|gif);base64,/i.test(image)&&!/^https?:\/\//i.test(image))return{error:'Foto inválida.'};const tech=type==='tecnologia'?technologyCustomization(b?.technologyCustomization):null;const clothing=type==='roupa'?clothingCustomization(b?.clothingCustomization):null;return{product:{id:productId(),name,description,price:Math.round(price*100)/100,type,effects:type==='consumivel'?effects(b?.effects):{},image,emoji,technologyCustomization:tech,clothingCustomization:clothing,vehicleCustomization:type==='veiculo'?vehicleCustomization(b?.vehicleCustomization):null,createdAt:new Date().toISOString()}}};
