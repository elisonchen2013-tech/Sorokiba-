(()=>{
  ...
  async function render(){
    if(!location.pathname||document.body.dataset.sorokibaDeleteLoaded)return;
    document.body.dataset.sorokibaDeleteLoaded='1';
    const observer=new MutationObserver(async()=>{
      const title=$('#pageTitle');
      if(!title||title.textContent!=='Prefeitura'||$('#accountDeletePanel'))return;
      try{
        ...
        const content=$('#content');content.appendChild(panel);
  ...
  window.sorokibaDeleteAccount=async(encoded,name)=>{
    ...
    try{const d=await api(`/api/mayor/users/${encodeURIComponent(username)}`,{method:'DELETE'});alert(d.message||'Conta excluída.');const list=document.querySelector('#accountDeleteList');if(list)list.parentElement.parentElement.remove();if(typeof loadPage==='function')loadPage('mayor')}catch(e){alert(e.message)}
  };
  render();
})();
