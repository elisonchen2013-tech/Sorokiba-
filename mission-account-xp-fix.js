...
app.post('/api/mayor/rewards',(req,res)=>{
 ...
 if(!city.missionRewards)city.missionRewards={};
 for(const job of jobs){const v=updates[job.id];if(!v)continue;const money=Number(v.moneyPerMission),xp=Number(v.xpPerMission),questions=Number(v.questionsPerMission);if(!Number.isFinite(money)||money<0||!Number.isFinite(xp)||xp<0||!Number.isInteger(questions)||questions<1||questions>20)return res.status(400).json({error:'Valores de recompensa inválidos.'});city.missionRewards[job.id]={moneyPerMission:Math.floor(money),xpPerMission:Math.floor(xp),questionsPerMission:questions};}
 saveData();
...
app.delete('/api/mayor/users/:username',(req,res)=>{
 ...
 const target=users[username];if(!target)return res.status(404).json({error:'Conta não encontrada'});
 if(target.isMayor)return res.status(400).json({error:'A conta do prefeito não pode ser excluída pelo painel.'});
 const counted=!!target.countedInPopulation;delete users[username];if(counted)city.population=Math.max(0,Number(city.population||0)-1);if(Array.isArray(city.proposals))city.proposals=city.proposals.filter(p=>p.authorUsername!==username);saveData();
 res.json({ok:true,message:`A conta ${target.name||username} foi excluída permanentemente.`});
});
app.delete('/api/me/account',(req,res)=>{
 ...
 const username=req.user.username,wasMayor=!!req.user.isMayor,counted=!!req.user.countedInPopulation;
 delete users[username];if(counted)city.population=Math.max(0,Number(city.population||0)-1);if(Array.isArray(city.proposals))city.proposals=city.proposals.filter(p=>p.authorUsername!==username);
 if(wasMayor){const remaining=Object.values(users);remaining.forEach(u=>u.isMayor=false);if(remaining.length)remaining.sort((a,b)=>String(a.createdAt||'').localeCompare(String(b.createdAt||'')))[0].isMayor=true;}
 saveData();
